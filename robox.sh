...
"generic-docker.json generic-hyperv.json generic-vmware.json generic-libvirt.json generic-libvirt-x32.json "\
"developer-ova.json developer-hyperv.json developer-vmware.json developer-libvirt.json"
...
function cache {
  ...
  if [[ $OS == "Windows_NT" ]]; then
    ...
    packer build -on-error=cleanup -color=false -debug -parallel-builds=$PACKER_MAX_PROCS -except= packer-cache.json 2>&1 | tr -cs [:print:] [\\n*] | grep --line-buffered --color=none -E "Download progress|Downloading or copying|Found already downloaded|Transferred:|[0-9]*[[:space:]]*items:"
  fi
